...
$this->title =$model->name." ".$model->nip;
...
$( document ).ready(function() {
	$('#<?php echo Html::getInputId($info, "photo"); ?>').bootstrapFileInput();
...
<div class="col-xs-12 col-lg-12">
  <div class="box box-info box-solid view-item col-xs-12 col-lg-12 no-padding">
   ...
   <div class="box-body">
	...
	<?php echo Html::img($info->getPhoto($info->photo), ["width"=>"150px", "height"=>"150px", 'alt'=>'No Image', 'id'=>'aImgShow']); ?>  
    </div>

    <div class="col-xs-12 col-sm-12 col-lg-12 text-center no-padding"> 
	<?= $form->field($info,'photo')->fileInput(['data-filename-placement' => "inside", 'title' => Yii::t('stu', 'Browse Photo'), 'onchange'=>'uploadImage(this)', 'data-target'=>'#aImgShow']) ?>
    </div>

    <div class="hint col-xs-12 col-sm-12" style="color:red;padding-top:15px"><b><?php echo Yii::t('app', 'Note'); ?>:- </b>&nbsp;<?php echo Yii::t('emp', 'Only Upload Jpeg, Jpg, Png, Gif Type'); ?>
    </div>
 
    <div class="form-group col-xs-12 col-sm-12 no-padding">
	<hr>
        <?= Html::submitButton($model->isNewRecord ? Yii::t('app', 'Create') : Yii::t('emp', 'Update'), ['class' => $model->isNewRecord  ? 'btn btn-success' : 'btn btn-info']) ?>
    </div>
    <?php ActiveForm::end(); ?>   
